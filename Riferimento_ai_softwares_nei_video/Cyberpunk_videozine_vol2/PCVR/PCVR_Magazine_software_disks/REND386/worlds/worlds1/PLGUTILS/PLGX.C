...
process_plg()		/* pocess all polys in object */
{
 ...
 if(centerx || centery || centerz ||
    groundx || groundy || groundz    )   /* precenter or shift vertices */
   for(i=0;i<nverts;i++)
    {
     if(centerx) v[i].x -= cx;
     if(centery) v[i].y -= cy;
     if(centerz) v[i].z -= cz;
     if(groundx) v[i].x -= minx;
     if(groundy) v[i].y -= miny;
     if(groundz) v[i].z -= minz;
    }
 ...
 if(laxis)	/* longest-axis fit scaling */
  {
   float l = 0;
   sx = sy =sz = 1.0;
   ...
   if(fabs(maxx-minx)>l)
    {
     l = maxx-minx;
     if(maxx-minx>0.0001) sx = laxis/(maxx-minx); else sx = 1.0;
     sy = sz = sx;
    }
   if(fabs(maxy-miny)>l)
    {
     l = maxy-miny;
     if(maxy-miny>0.0001) sy = laxis/(maxy-miny); else sy = 1.0;
     sx = sz = sy;
    }
   if(fabs(maxz-minz)>l)
    {
     if(maxz-minz>0.0001) sz = laxis/(maxz-minz); else sz = 1.0;
     sx = sy = sz;
    }
  }
 ...
 if(clim>0.0)               /* delete duplicte vertices */
  {
   ...
   fprintf(stderr," %d duplicate vetrices found: deleted!\n", nverts-nvtx);
   nverts = nvtx;
 ...
 if(newcolor[0])
  for(i=0;i<npolys;i++)    /* update polygon colors */
   {
    if(p[i]==NULL) continue;
    strcpy(p[i]->color, newcolor);
   }

 if(rrecolor)
  {
   for(i=0;i<npolys;i++)    /* update polygon colors randomly */
    {
     if(p[i]==NULL) continue;
     rndcolor[3]++;
     if(rndcolor[3]>'F') rndcolor[3] = '1';
     if(rndcolor[3]>'9' && rndcolor[3]<'A') rndcolor[3] = 'A';
     strcpy(p[i]->color, rndcolor);
    }
  }
}
...
syntax()
{
 fprintf(stderr,"PLGX.EXE PLG file processor (c) 1993 by Dave STampe\n");
     ...
     switch(toupper(argv[i][1]))
      {
       case 'C':/* center overall */
		if(!argv[i][2]) centerx = centery = centerz = 1;
		else
		 {
		  centerx = strchr(argv[i]+2,'x') ? 1 : 0;
		  centery = strchr(argv[i]+2,'y') ? 1 : 0;
		  centerz = strchr(argv[i]+2,'z') ? 1 : 0;
		 }
		break;
       case 'G':/* ground  */
		if(!argv[i][2]) groundx = groundy = groundz = 1;
		else
		 {
		  groundx = strchr(argv[i]+2,'x') ? 1 : 0;
		  groundy = strchr(argv[i]+2,'y') ? 1 : 0;
		  groundz = strchr(argv[i]+2,'z') ? 1 : 0;
		 }
		break;
       ...
       case 'N':/* renumber vertices in comments */
		renverts = 1;
		break;
       case 'Q':/* recolor all polys randomly */
		rrecolor = 1;
		break;
       ...
       case 'S':/* scale PLG */
		...
		break;
       case 'P':/* recolor all polys */
		strcpy(newcolor, argv[++i]);
		break;
