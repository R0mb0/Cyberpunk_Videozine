                TELIX-SQUARE package by Inge Vabekk.
 
     NB.! NB.! NB.! NB.! NB.! NB.! NB.! NB.! NB.! NB.! NB.! NB.!
   ษอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป
   บ                     THIS IS VERSION 2.1แ.                   บ
   บ I will appreciate your feedback,  whether you are satisfied บ 
   บ or not!  Messages can  be sent  to me  via private  mail at บ
   บ Non Plus Ultra MBBS, (472) 46 51 27 / 46 39 51, in the      บ
   บ Telix_Square conference. I occasionally also call other     บ
   บ systems, but if you live far from Oslo you can call a       บ
   บ system with PCrelay connection. Route the mail to ->CAVE.   บ
...
NOTE:
  ...
  packed manually.  The programs PUSHDIR.COM and POPDIR.COM must also
  be moved to a directory that's included in your PATH.
  If you have 4dos or another PUSH/POPD program system,  you can just
  delete them, or change FONE.BAT to use your own versions.
...
FONE.BAT     Batch file to start TELIX.
             ...
             EXAMPLES:  c:\> Fone non plus   to call Non Plus Ultra,
                        c:\> Fone @BBS       to call all BBSes listed
                                             in the file BBS.

             If you want to  use another phone directory,  the option 
             F<phonedir> can be added to the TELIX run command.  This
...
Timed start: If you want dialling to start at a specific time,  enter
             ...
             Two digits for the hour,  the time separator (defined in
             the Telix configuration menu!),  and two digits  for the
             minute. Characters in addition to these will be ignored.
             Note that if you specify a time after 12:00 you must use
             24-hour time format in Telix to make it work.
...
DUPECHK$.EXE Program to check for potential duplication of uploads.
             All files in your pushlist are checked against the files
             in the BBS file list. Existing files are NOT uploaded to
             the same BBS again, except if they're marked for private
             upload. The filename is added to the existing file list.

             You should have one file list for each BBS you call. The
             name of this file is derived from the BBS name  by using
             the first eight alphabetic characters (a-z) (maximum). 
             There must of course be at least one legal character. 
             ...
             you should check that the filename is listed in the file
             FILELIST.BBS.  This allows you to fetch the same file if
             you need it again later.

             The DUPECHK$ program checks  that the files can be found
             in either the upload or download directory. If necessary
             the full path of the file is used in the  upload command
             file (<BBS>.UL).


HAVEIT$.EXE  Program to check if a  BBS file list contains  the files
             listed in the WISH.LST file.  Files that are also listed
             in the log file  DOWNLOAD are removed before this compa-
             rison is done. 
             ...
             Don't write "Haveit$ Non Plus Ultra MBBS"
             but  write  "Haveit$ NONPLUSU" .  (capitals not needed).

             The file NONPLUSU.DL (for the above example) is created,
...
COMPILE.EXE  Program for quick and easy compile of SALT scripts. Move
             ...
             piler either in the same directory or in your PATH.
...
QDIAL.SLC    when TELIX is started from the FONE.BAT command file. If
             ...
             Type 5 (five) or more characters for the BBS name to put
             ALL matches found into the dial list.
             If you specify less than 5,  you'll have to confirm each
...
LOGON.SLC    strings used in the following BBS systems:
             ...
             A set of softkeys, defined in the NAME.DAT file, are set
             up to schedule these scripts.
...
DIALBACK.SLT This script is usable for PCB systems that offer a spec-
DIALBACK.SLC ial service to some user:  Dialback means  that you call
             the BBS to identify yourself and give your phone number.
             The script then hangs up, waiting for the phone to ring.
             The BBS software calls the given number,  and the script
             reconnects.  Thus, the owner of the BBS pays the rest of
             the phone call.
...
MAILUP.SLC   a special format, equal to the format used by the XBoard
             ...
             MBBS ONLY:
             If you write a message to the "Resum" conference,  or a
             conference having the word "resum"  as part of the name,
             the MailUp script will use a special command,  "Q EDIT",
             to enter the MBBS editor in the special User Information
             conference which is defined on  (almost?)  all MBBS sys-
             tems.
             Write your own user information file, and import it into 
             a message for uploading the next time you're online.
             Don't worry about  sending it to the right person.  This
...
UPLOAD.SLC   listed in the file PUSH.LST, which must be in the  TELIX
             ...
             P:Username         (means that next file is private)
             F:filename 
             ...
             Any line starting with something other than P:, F: or D:
             is ignored, and so are all lines until another P:, F: or
             D: line is found.  Thus you may exclude a file  from the
             download list by changing F: to for instance X: .
...
DNLOAD.SLC   WISH.LST, which is read by the separate program HAVEIT$.
             ...
             Anyway, ÿonly the first "MAXDN" files  will be requested
             for download in each run.  MAXDN is defined as a keyword
             in the NAME.DAT file.
             ...
             +<conference>       Join <conference> permanently.  Next
                                 time you log on to the same BBS this
                                 conference  will be scanned for mes-
                                 sages.
             -<conference>       Resign <conference> permanently. For
                                 PCB systems  the  next file will  be
                                 ...
                                 another conference before next down-
                                 load.
...
GRABMAIL.SLC the LOGON script to a BBS.  This script  performs a GRAB
             on an MBBS system,  and a R A Z command on PCBoard. This
...
NEWFILES.SLT Script used automatically after three days without upda-
NEWFILES.SLC ting your file list  on the current BBS.  The script can
             also be scheduled manually in case you want an immediate
             update.


TAGLINE.SLT  This script is used to send your tagline, trigged either
TAGLINE.SLC  by  a softkey  (if defined in NAME.DAT),  or by the text
             "!TAG"  (quotes not needed)  in your  upload mail  (when 
             using ASCII upload).  Note that you may have three lines
             for a tagline,  where "TAGA" and "TAGC"  are used if you
             are connected to a PCBoard system.
             If you are connected to  any other type of system, lines
...
JOINCONF.SLC can only be called from other scripts.  Obviously, it is
             ...
             The script keeps track of the conference you are in, and
             avoids switching to the current one.
...
DISPLAY.SLC  for your Telix_Square package. Can be executed any time,
             and is called automatically by several other scripts.
...
NAME.DAT     This name file  is needed  for all the scripts  used for
             ...
             mdir:  MAIL download directory.  If not specified,  mail
                    is downloaded to the file D/L directory specified
                    in the Telix configuration.
             ...
             NOTE: If you use VT102 emulation, keys F1-F4 won't work.
             Use ANSI-BBS!
...
FILELIST.BBS This file is used to keep the name  of the file list for
  ...
  own testing, however, shows that the MobyTurbo protocol only offers
  a 3% improvement over internal Zmodem,  and internal Ymodem-G (with
  MNP modem only!) is even better.
  ...
  Telix_Square does not add a port number.  By allowing you to define
  it yourself,  other than standard settings  can be supported  (i.e. 
  port 3, IRQ 5: portx 3e8,5)
  ...
  I hereby donate the Telix_Square package to the Public Domain.  All
  script sources  are supplied.  I have attempted  to make them clean 
  and easy to read, with plenty of comments.  This should help you to
  make your own adjustments,  and to resolve  any problems that might
  appear (but please give me a feeback!).
